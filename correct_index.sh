...
for j in $(ls *fastq.gz *fq.gz | grep -v "exact") ; do i=`echo $j | cut -f1 -d'.'`; rm ${i}_exact.txt;
	...
	echo "zcat ${j} | grep \"@*$INDEX\" | cut -f1 -d' ' | cut -f2 -d'@' >> ${i}_exact.txt ; ~/programs/seqtk/seqtk subseq ${j} ${i}_exact.txt | gzip -c - > ${i}_exact.fastq.gz && rm ${i}_exact.txt  " >> index_parallel.txt; done
